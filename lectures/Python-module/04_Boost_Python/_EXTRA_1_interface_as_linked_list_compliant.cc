...
BOOST_PYTHON_MODULE(_EXTRA_1_interface_as_linked_list_compliant) {
	class_<List <int>, boost::noncopyable >("List")
	.def("__iter__" , iterator<List<int>>())
	.def("insert"   , &List<int>::insert,insert_overloads())
	.def("print"    , &List<int>::print)
	;
}
